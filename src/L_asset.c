...
void load_animation(const char* name) {
    ...
    void* buffer = SDL_LoadFile(file, NULL);
    if (buffer == NULL) {
        WTF("Animation \"%s\" load fail: %s", name, SDL_GetError());
        return;
    }
    uint8_t* cursor = (uint8_t*)buffer;
    ...
    // Data
    enum BoneSpaces spaces = read_u8(&cursor);
    animation->num_frames = read_f64(&cursor);
    animation->frame_speed = read_f64(&cursor) / (double)TICKRATE;

    animation->num_nodes = 0; // read_u32(&cursor);
    animation->num_bones = 0; // read_u32(&cursor);
